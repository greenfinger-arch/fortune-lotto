    ...
    <div className="w-full min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-4 relative font-sans">
      ...
      <div className="max-w-[400px] w-full bg-slate-900/70 backdrop-blur-3xl rounded-[2.5rem] shadow-2xl border border-white/10 relative z-10 overflow-hidden my-auto max-h-[95vh] flex flex-col">
        <header className="pt-8 pb-4 text-center flex-shrink-0">
          ...
          <h1 className="text-lg font-black tracking-[0.2em] text-white uppercase italic">Fortune Lotto 2026</h1>
          <p className="text-[9px] text-slate-500 mt-1 tracking-widest opacity-60 uppercase">Rank Lamp Lab</p>
        </header>
          ...
          <AnimatePresence mode="wait">
            {step === 1 && (
              <motion.div key="step1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
                <section>
                  <h2 className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-3 text-center">오행 에너지 선택</h2>
                  <div className="grid grid-cols-5 gap-2">
                    {Object.keys(FENG_SHUI_ELEMENTS).map((key) => (
                      ...
                      >
                        ...
                        <span className="text-[8px] mt-0.5 font-medium">{FENG_SHUI_ELEMENTS[key].name.split(' ')[1]}</span>
                      </button>
                    ))}
                  </div>
                </section>
                ...
                <section>
                  <h2 className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-3 text-center">생기 집중 공간</h2>
                      ...
                      >
                        <div className="flex items-center gap-3 text-left">
                          ...
                          <div>
                            ...
                            <p className="text-[9px] text-slate-500 leading-none mt-1">{spot.desc}</p>
                          </div>
                        </div>
            ...
            {step === 2 && (
              <motion.div key="step2" className="py-16 text-center relative flex flex-col justify-center items-center">
                <canvas ref={canvasRef} className="absolute inset-0 w-full h-full pointer-events-none" />
                <div className="relative z-10">
                  <motion.div animate={{ opacity: [0.5, 1, 0.5] }} transition={{ repeat: Infinity, duration: 1.5 }} className="text-2xl font-black tracking-[0.4em] text-white uppercase">Analyzing</motion.div>
                  <p className="text-slate-500 text-[10px] mt-4 tracking-widest uppercase">데이터 행운군 추출 중...</p>
                </div>
              </motion.div>
            )}
            ...
            {step === 3 && (
              <motion.div key="step3" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="text-center space-y-6 pt-2">
                <div className="p-6 bg-slate-950/40 rounded-[2rem] border border-white/5 shadow-inner">
                  <h3 className="text-[9px] font-bold text-slate-600 tracking-[0.3em] uppercase mb-6">SELECTED LUCKY NUMBERS</h3>
